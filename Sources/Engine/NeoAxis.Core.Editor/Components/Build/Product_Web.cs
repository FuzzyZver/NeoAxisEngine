	...
	public class Product_Web : Product
	{
		...
		public override void BuildFunction( ProductBuildInstance buildInstance )
		{
			...
			try
			{
				...
				buildInstance.Progress = 0.35f;

				if( CheckCancel( buildInstance ) )
					return;

				BuildProjects( buildInstance, dotnetExePath, publishFolder, new Range( 0.35, 0.85 ) );
		...
		{
			var items = new string[] { "Assets", "Caches", "Binaries", "Sources", "Properties", "Build.Web.sln", "Build.Web.Extended.sln", "Project.Web.csproj" };
